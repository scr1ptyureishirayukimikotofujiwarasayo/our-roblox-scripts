...
local Minimize = Instance.new("TextButton") -- Added minimize button
...
Minimize.Parent = Top
Minimize.BackgroundColor3 = Color3.new(0.588235, 0.0196078, 0.172549)
...
Minimize.MouseButton1Down:connect(function()
	if not isMinimized then
		-- Minimize
		Base.Size = UDim2.new(0, 450, 0, 35)
...
function topkek.libutil:recurseDecal(img) -- topkek2.0 code tbh
	...
	local function recurse(x)
		for i, v in pairs(x:GetChildren()) do
			pcall(function() -- 'error occured, no Lua output' LOLE
...
function topkek.libutil:insert(id)
	if topkek.cache[id] then return topkek.cache[id] end -- wheet
...
function topkek.libutil:weiner(plr)
	...
	-----
	local Ball1 = Instance.new("Part", plr)
	Ball1.Name = 'Ball1'
	Ball1.Size = Vector3.new(1, 1, 1)
	Ball1.TopSurface = 0
	Ball1.BottomSurface = 0
	local Cyln3 = Instance.new("SpecialMesh", Ball1)
	...
	-----
	local Ball2 = Instance.new("Part", plr)
	Ball2.Name = 'Ball2'
	Ball2.Size = Vector3.new(1, 1, 1)
	Ball2.TopSurface = 0
	Ball2.BottomSurface = 0
	local Cyln3 = Instance.new("SpecialMesh", Ball2)
	Cyln3.MeshType = 'Sphere'
	Cyln3.Scale = Vector3.new(0.4,0.4,0.4)
...
servwin:drawButton(1/2, '666', function()
	for i, v in next, workspace:children''do
	    ...
	    --coroutine.wrap(function() while wait
	      s = Instance.new'Sound';
	      s.Parent = workspace;
...
servwin:drawButton(1/2, 'Colorize', function() -- when you slide the variable XDDDDDpranked
	local material = 
	...
	local function r(where) 
		for _, v in pairs(where:GetChildren()) do 
			if v:IsA("BasePart") then 
				spawn(function() 
					while wait(0.1) do -- Added 'do' here
						v.Material = material[math.random(#material)] 
						wait() 
					end 
				end) 
			end 
