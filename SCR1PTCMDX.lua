if IY_LOADED and not _G.IY_DEBUG then
	-- error("Scr1ptCMDX is already running!", 0)
	return
end
...
Title.TextSize = 18
Title.Text = "Scr1ptCMDX v" .. currentVersion
...
addcmd('clientantikick',{'antikick'},function(args, speaker)
	...
	if hookfunction then
		...
		notify('Client Antikick','Hooked Kick method - kick attempts will be blocked')
	elseif setmetatable then
		LocalPlayer.Kick = function() end
		notify('Client Antikick','Replaced Kick method - kick attempts will be blocked')
	else
		notify('Client Antikick','No compatible method available for this exploit')
	end
end)
...
addcmd('clientantiteleport',{'antiteleport'},function(args, speaker)
	...
	if hookfunction then
		hookfunction(TeleportService.Teleport, function() return nil end)
		hookfunction(TeleportService.TeleportToPlaceInstance, function() return nil end)
		notify('Client AntiTP','Hooked teleport methods - teleport attempts will be blocked')
	else
		TeleportService.Teleport = function() end
		TeleportService.TeleportToPlaceInstance = function() end
		notify('Client AntiTP','Replaced teleport methods - teleport attempts will be blocked')
	end
end)
	...
	for x = 0, 1, 0.5 do
		for y = 0, 1, 0.5 do
			...
			if raycastResult then
				dist = (raycastResult.Position - origin).magnitude
			end
...
local antiflingMainConn = nil
local antiflingCharConns = {}

local function disablePlayerCollisions(player, speaker)
	if player == speaker or not player.Character then return end
...
addcmd("antifling", {}, function(args, speaker)
	...
	-- Disable collisions for existing players
	for _, player in pairs(Players:GetPlayers()) do
		disablePlayerCollisions(player, speaker)
	end
	
	-- Setup connections for new players and respawns
	antiflingMainConn = Players.PlayerAdded:Connect(function(player)
		...
		if not antiflingCharConns[player] then antiflingCharConns[player] = {} end
		table.insert(antiflingCharConns[player], charConn)
		disablePlayerCollisions(player, speaker)
	end)
	
	-- Setup for existing players' respawns
	for _, player in pairs(Players:GetPlayers()) do
...
addcmd("unantifling", {"noantifling"}, function(args, speaker)
	antiflingActive = false
	if antiflingMainConn then
		antiflingMainConn:Disconnect()
		antiflingMainConn = nil
	end
	for player, conns in pairs(antiflingCharConns) do
...
addcmd("toggleantifling", {}, function(args, speaker)
	execCmd(antiflingActive and "unantifling" or "antifling")
end)
